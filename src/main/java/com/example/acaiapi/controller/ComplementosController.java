...
import com.example.acaiapi.repository.ComplementosRepository;
...
public class ComplementosController {
    ...
    private final ComplementosRepository repository;

    public ComplementosController(ComplementosRepository repository) {
        this.repository = repository;
    }

    @GetMapping("/")
    public List<Complementos> listarComplementos() {
        return repository.findAll();
    }

    @PutMapping("/{id}/ativo")
    public ResponseEntity<?> atualizarAtivo(@PathVariable Long id, @RequestBody Map<String, Boolean> body) {
            Optional<Complementos> ComplementoOpt = repository.findById(id);
            ...
            Complementos Complemento = ComplementoOpt.get();
